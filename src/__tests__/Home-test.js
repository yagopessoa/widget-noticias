...
import { Title, Button } from '../Components/baseComponents'
...
describe('components', () => {
  describe('Home', () => {
    it('should render self and subcomponents', () => {
      ...
      expect(enzymeWrapper.find(Title).text()).toBe('Notícias')
      expect(enzymeWrapper.find('option').text()).toBe('Filtrar por fonte')
      expect(enzymeWrapper.find(Button).text()).toBe('Mostrar mais')

    })

    it('should call fetchNews() when clicking button', () => {
        const { enzymeWrapper } = setup()

        fetchMock.mock(`https://newsapi.org/v2/top-headlines?country=br&pageSize=5&page=1&apiKey=${process.env.REACT_APP_API_KEY}`, {
          body: {
